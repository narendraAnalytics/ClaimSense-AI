...
## Current status
Backend is past the scaffold stage. Phase 3 (FastAPI foundation: Pydantic Settings config, loguru logging, constants/enums, custom exceptions, health endpoint) and Phase 4 (claim intake: Pydantic schemas/domain models, an in-memory claim registry, a local-disk upload service, `POST /claims` and `POST /claims/{claim_id}/upload`) are both implemented and pushed. `SARVAM_API_KEY` is present in `backend/.env` but still nothing calls the Sarvam API — no LLM/OCR calls, no Convex/DB persistence, and no LangGraph wiring exist yet. Phase 5 (per `projectfolder.txt`) is next. Treat any claim about "what the app does" with suspicion until verified by reading actual code — the docs describe the target, and even this status note can drift from what's actually implemented.

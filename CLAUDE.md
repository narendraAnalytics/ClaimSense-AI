...
## Current status
Backend is past the scaffold stage. Phase 3 (FastAPI foundation), Phase 4 (claim intake: `POST /claims`, `POST /claims/{claim_id}/upload`), Phase 5 (LangGraph orchestration skeleton with a `workflow.py` service gating all graph access, verified end-to-end by a pytest regression test), Phase 6 (Document Intelligence Agent: real OCR via Sarvam Vision, the first real Sarvam API call, live-verified against a real HDFC ERGO claim form — not mocked), Phase 7 (Policy Coverage Agent: the first real Sarvam-30B reasoning call, live-verified against a real sample policy document — not mocked), and Phase 8 (Medical Validation Agent: the second real Sarvam-30B reasoning call, cross-validating diagnosis/treatment across a discharge summary, prescription, and lab report, live-verified against a real synthetic patient's hospital documents — not mocked) are implemented and pushed. The graph runs `START -> document -> supervisor -> policy -> medical -> END`: uploaded documents get OCR'd into structured JSON, the Policy agent calls Sarvam-30B to extract structured policy fields and a coverage decision, then the Medical agent calls Sarvam-30B again to validate clinical consistency across whichever medical documents were attached. Still no Convex/DB persistence, no graph-level checkpointing (re-processing a claim re-runs OCR *and* policy extraction *and* medical validation from scratch and spends real Sarvam credits), and only 5 of the 9 graph nodes are actually wired into the compiled graph — Billing, Fraud, History, Settlement, and Report are still placeholder stubs with no reasoning calls. Phase 9+ (per `projectfolder.txt`) is next: real reasoning for the Billing/Fraud agents. Treat any claim about "what the app does" with suspicion until verified by reading actual code — the docs describe the target, and even this status note can drift from what's actually implemented.
